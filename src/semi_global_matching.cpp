...
bool SemiGlobalMatching::Initialize(const int32_t& width, const int32_t& height, const SGMOption& option)
{
    ...
    if (option.census_size == Census5x5) {
        ...
        census_left_ = new uint64_t[img_size]();
        census_right_ = new uint64_t[img_size]();
    }
...
void SemiGlobalMatching::CensusTransform() const
{
    ...
    if (option_.census_size == Census5x5) {
        sgm_util::census_transform_5x5(img_left_, static_cast<uint32_t*>(census_left_), width_, height_);
        sgm_util::census_transform_5x5(img_right_, static_cast<uint32_t*>(census_right_), width_, height_);
    }else {
        sgm_util::census_transform_9x7(img_left_, static_cast<uint64_t*>(census_left_), width_, height_);
        sgm_util::census_transform_9x7(img_right_, static_cast<uint64_t*>(census_right_), width_, height_);
    }
}
        ...
        for (int32_t j = 0; j < width_; j++) {
        	...
        	for (int32_t d = min_disparity; d < max_disparity; d++) {
                ...
                if (option_.census_size == Census5x5) {
                    ...
                    const auto& census_val_l = static_cast<uint64_t*>(census_left_)[i * width_ + j];
                    const auto& census_val_r = static_cast<uint64_t*>(census_right_)[i * width_ + j - d];
